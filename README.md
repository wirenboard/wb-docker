...
| 4 | Наш пакет удалён через `apt remove` (без purge), ставим снова | То же: dpkg сообщает нашу прежнюю версию, раскладка на месте | Ничего |
...
| 7 | Стоял `docker-ce` не из WB, удалили через `apt remove` (без purge), ставим наш | Тоже пропускает настройку: dpkg сообщает чужую прежнюю версию | `wb-docker-setup`, если нужна раскладка WB |
...
Поэтому настройка выполняется только в случаях 1–5 из таблицы: первая установка
пакета или обновление нашего же пакета там, где раскладка уже применена. В
остальных случаях в лог `apt` идёт строка о том, что настройка пропущена, и
команда, которой её применяют вручную.
