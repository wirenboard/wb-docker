...
Поэтому `postinst` настраивает только два случая: первая установка пакета и
обновление нашего же пакета там, где раскладка уже применена. Docker,
поставленный вручную или community-скриптом, остаётся как есть — в лог `apt`
пишется, что настройка пропущена.

Перейти на WB-раскладку осознанно:

```bash
wb-docker-setup       # спросит подтверждение, если в /var/lib/docker есть данные
```

Команда делает то же, что `postinst` на чистой установке, и перезапускает
`docker.service`. Существующий `daemon.json` она не перезаписывает — и если в нём
нет `data-root`, скажет об этом: данные останутся в `/var/lib/docker`.

### Что делать с данными прежнего Docker
